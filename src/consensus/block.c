                          ...
                          block_t *block, size_t *consumed)
{
    ...
    result = varint_read(data + offset, data_len - offset, &tx_count, &var_consumed);
    if (result != ECHO_OK) {
        return result;
    }
    offset += var_consumed;
    ...
    for (i = 0; i < block->tx_count; i++) {
        ...
        result = tx_parse(data + offset, data_len - offset,
                          &block->txs[i], &tx_consumed);
        if (result != ECHO_OK) {
            block_free(block);
            return result;
        }
                              ...
                              uint8_t *buf, size_t buf_len, size_t *written)
{
    ...
    for (i = 0; i < block->tx_count; i++) {
        ...
        result = tx_serialize(&block->txs[i], ECHO_TRUE,
                              buf + offset, buf_len - offset, &tx_written);
        if (result != ECHO_OK) {
            return result;
        }
        offset += tx_written;
